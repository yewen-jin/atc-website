...
- **Livi Room:** `artists/livi/` has its own frame-based layout. The desktop page uses `artists/livi/assets/frames/background.png` as the page background and `artists/livi/assets/frames/frame.png` as a non-interactive chrome overlay on `.livi-console::after`. The three nav buttons (`Gallery`, `Tablet`, `About`) are absolutely positioned inside the left frame holes using `button-1.png`, `button-2.png`, and `button-3.png`. The content fill sits below the chrome via z-index, and `.console-title` sits above the frame at the top. The fixed top-left `.back-link` home button reuses the same pushbutton language with `button-4.png`; on mobile the pill nav is pushed down by `--sp-9`, which is derived from that button's own `clamp(58px, 13vw, 76px)` width plus a gap so the nav sits as high as it can without ever colliding with it — adjust the gap term rather than hardcoding a new offset. The Gallery uses CSS thumbnails and `left.png` / `right.png` image controls, with keyboard ArrowLeft/ArrowRight support in the inline script. Thumbnail buttons hug their image — the strip is a centred flex row and `.thumb-btn` sets only a height (64px desktop / 52px mobile) with `width: auto` on the `img`, so each button is exactly 12px wider and taller than its thumbnail. The thumbnail `<img>` tags carry `width`/`height` attributes: without them the lazy images have no intrinsic ratio and the buttons collapse to 12px wide until they load. On mobile the arrow controls leave the image and join the thumbnail row: `.slide-stage` goes `display: contents` so the controls and the strip become siblings in a three-column `.slideshow` grid (arrow / strip / arrow), and `align-self: center` keeps the arrows on the strip's midline even when the thumbnails wrap to two rows on narrow phones — don't go back to absolute `bottom: 0`, which pins them to the last wrapped row. Gallery images are never cropped: `.slide-image` and `.thumb-btn img` both use `object-fit: contain`, and their boxes are pinned to `aspect-ratio: 16 / 9` — wider than the widest photo in the set (1.70) — so every slide and every thumbnail is height-limited and they all draw at the same height. Keep any new gallery image narrower than 16:9, or widen those ratios to match. `.slide-frame` carries no padding or background — the slide image fills it exactly, and its `border-radius` rounds the photo itself. `.slide-stage` wraps the figure and the two arrow controls and hugs the slide, so the controls' `top: 50%` centres on the image rather than on the whole slideshow block (which includes the thumbnail strip) — keep them inside it when editing the markup. Below `980px` the slideshow is `height: auto` with `padding: 0` and `.slide-frame` is full width, so the slide spans the card's inner width and the arrow controls ride over its edges; mobile deliberately inverts the desktop rule — `.slide-image` drops its `aspect-ratio`/`max-height` there so every slide is the same **width** and its height follows its own ratio (the card grows and shrinks as you step between landscape and portrait shots), while desktop keeps the same **height**; `.content-area` drops its `min-height` at both mobile breakpoints so the card hugs whichever section is active instead of holding a fixed floor. The Tablet section is a `.tablet-layout` two-column grid — copy plus the `.tablet-pair` "bean guys" on the left, the `.image-pair` install view stacked on the right, both inside the one scrolling `.text-panel`, with the quotes spanning below; under `980px` it collapses to one column, and the install pair takes `order: -1` so it sits above the copy and goes back to a side-by-side row (DOM order stays copy-first, which is what desktop and screen readers follow). Both install shots print at one shared width (`.image-pair .image-figure` is `min(100%, 300px)`, image `width: 100%; height: auto`), so the landscape and portrait photos line up on both edges and only their heights differ — `.installation-view .featured-image` sets `max-height: none` to lift the base `.featured-image` cap. `.text-panel` styles its own chrome-rail scrollbar through `::-webkit-scrollbar`; the Firefox `scrollbar-width`/`scrollbar-color` fallback is deliberately wrapped in `@supports not selector(::-webkit-scrollbar)`, because setting those in Chrome switches the gradient thumb off. Image captions are not rendered in Livi's room — each `img` carries a descriptive `alt` that absorbs what the caption used to say (the lightbox keeps its own `.lightbox-caption`).
