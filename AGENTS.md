...
- **About Page:** `about/index.html` serves at `/about`. It reuses the same two CSS files and the full HUD shell (background, inner backdrop, `frame-outer.png`, `frame-title.png`) but has no orbit panels. Page-specific layout overrides (wider/taller content area) live in an inline `<style>` block inside `about/index.html`. The logo on this page links back to `/`. Asset paths use `../assets/...`.
